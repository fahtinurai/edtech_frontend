// components/MateriRecommendation.jsx
import React, { useState } from "react";

const MateriRecommendation = () => {
const [form, setForm] = useState({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/rekomendasi-materi/rekomendasi", {
        method: "POST",
      ...
      setHasil(data.rekomendasi_materi);
    } catch (err) {
      setHasil("❌ Gagal mendapatkan rekomendasi.");
      console.error(err);
    }
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-xl font-semibold mb-4">🔍 Rekomendasi Materi</h2>
      ...
      {hasil && (
        <div className="mt-4 p-4 bg-green-100 text-green-700 rounded">
          🧠 <strong>Rekomendasi Materi:</strong> {hasil}
        </div>
      )}
    </div>
  );
...
export default MateriRecommendation;
